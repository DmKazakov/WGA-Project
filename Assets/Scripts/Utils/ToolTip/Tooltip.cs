...
public class Tooltip : MonoBehaviour
{
    ...
    public float speed = 0.0f; // скорость плавного затухания и проявления
    ...
    void LateUpdate()
    {
        ...
        if (show || isUI)
        {
            ...
            foreach (Image bg in img)
            {
                bg.color = Color.Lerp(bg.color, bg.color, speed * Time.deltaTime);
            }
            boxText.color = Color.Lerp(boxText.color, Color.white,  speed * Time.deltaTime);
        }
        else
        {
            foreach (Image bg in img)
